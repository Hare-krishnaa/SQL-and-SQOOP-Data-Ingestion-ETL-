...
CREATE TABLE IF NOT EXISTS `E_commerce_db`.`Shopping Cart` (
...
CREATE INDEX `ProductID_idx` ON `E_commerce_db`.`Shopping Cart` (`ProductID` ASC);
...
CREATE TABLE IF NOT EXISTS `E_commerce_db`.`view1` (`id` INT);


DROP TABLE IF EXISTS `E_commerce_db`.`view1`;
USE `E_commerce_db`;
